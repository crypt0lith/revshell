...
def map_reduce[_T, _R](
    __initial: _T, __func: Callable[[_T], _R], *funcs: Callable[[_R], _R]
) -> _R:
    return reduce(lambda x, f: f(x), [__func, *funcs], __initial)


def _compgen_path[AnyStr: (str, bytes)](__fp: os.PathLike[AnyStr] | AnyStr):
    fp = map_reduce(__fp, os.fsdecode, osp.expanduser, osp.abspath)
